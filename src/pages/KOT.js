...
function KOTPage() {
  ...
  // Group table and room orders together
  const groupedOrders = orders.reduce((acc, booking) => {

    const number = booking.number; // works for both room and table

    const key =
      booking.orderType === "room"
        ? `room-${number}`
        : `table-${number}`;
    ...
    acc[key].orders.push(...booking.orders);
